...
float* op_amp::sim(const int start, const int end, const float amplitude, const float frequency, const float phase, const float DC_offset) const {
    vector<float> output; //16 bit simulator so there will always be 512 data points.
    vector<float> time_array;
    const float data_gap = 1/(512*frequency); //distance between each data point (x-axis) (derived from (end - start)/512 = period / 512 =
    int k = 0;
    for(int i = start; i < start + 512; i++) {
        const float time = static_cast<float>(i) * data_gap;
        time_array.push_back(time);
        output.push_back(_gain*amplitude*sin((static_cast<float>(2*PI)*frequency*time)+phase)+DC_offset);

    }
    float* output_pointer = &output[0];
